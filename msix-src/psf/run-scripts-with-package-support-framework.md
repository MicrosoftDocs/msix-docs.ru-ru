...
|-------------------------|------------|-----------|----------|---------|
| `scriptPath`              | string     | Да       | Н/Д      | Путь к сценарию, включая имя и расширение. Путь задается относительно рабочего каталога приложения, если он указан, в противном случае он запускается в корневом каталоге пакета.
| `scriptArguments`         | string     | Нет        | empty    | Список аргументов, разделенных пробелами. Формат для вызова скрипта PowerShell одинаков. Эта строка добавляется к `scriptPath` для создания допустимого вызова PowerShell. exe.
