...
Обязательно включите в пакет файл Стартингскриптвраппер. ps1 и поместите его в ту же папку, что и исполняемый файл. Этот файл можно скопировать из [пакета NuGet ПСФ](https://www.nuget.org/packages/Microsoft.PackageSupportFramework/).
...
| `runInVirtualEnvironment` | Логический    | Нет        | верно     | Указывает, должен ли сценарий запускаться в той же виртуальной среде, в которой выполняется Пакетное приложение.
| `runOnce`                 | Логический    | Нет        | верно     | Указывает, должен ли сценарий запускаться один раз для каждого пользователя в каждой версии.
...
| `waitForScriptToFinish`   | Логический    | Нет        | верно     | Указывает, должно ли упакованное приложение ожидать завершения начального скрипта перед запуском.
