...
Дополнительные сведения см. в разделе [пакет поддержки пакетов](package-support-framework-overview.md).

Это средство поможет определить проблемы совместимости приложений, найти, применить и расширить исправления среды выполнения, которые их устраняют.
...
| исправлений | файле | Используемых Определяет, как ведет себя адресная привязка DL. Точный формат этого значения зависит от способа исправления, так как каждая адресная привязка может интерпретировать этот «большой двоичный объект». |
