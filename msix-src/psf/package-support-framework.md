...
* Разработчики: Несмотря на то, что разработчики могут найти всю полезную статью, наиболее релевантные разделы — это [Отладка, расширение или создание исправления среды выполнения](#debug-extend-or-create-a-runtime-fix), [Создание исправления среды выполнения](#create-a-runtime-fix)и [другие методы отладки](#other-debugging-techniques).
...
Откройте манифест пакета в текстовом редакторе, а затем присвойте `Executable` атрибуту `Application` элемента имя исполняемого файла средства запуска ПСФ.  Если вы знакомы с архитектурой целевого приложения, выберите соответствующую версию, PSFLauncher32. exe или PSFLauncher64. exe.  В противном случае PSFLauncher32. exe будет работать во всех случаях.  Ниже приведен пример.
...
| веб-масштабированием; | workingDirectory | Используемых Путь относительно пакета, используемый в качестве рабочего каталога запускаемого приложения. Если это значение не задано, операционная система использует `System32` каталог в качестве рабочего каталога приложения. |
| процессы | исполняемый файл | В большинстве случаев это будет имя `executable` , указанное выше, с удаленным путем и расширением файла. |
...
Откройте страницы свойств для проекта и на странице **Общие** параметры задайте для ``PSFLauncher32`` свойства ``PSFLauncher64`` **имя целевого объекта** значение или в зависимости от архитектуры приложения.
...
| Свойство. | Значение |
|-------|-----------|
| Копировать локально | Да |
| Копировать локальные вспомогательные сборки | Да |
| Выходные данные ссылочной сборки | Да |
...
Объявите ``FIXUP_DEFINE_EXPORTS`` макрос, а затем добавьте оператор include для элемента `fixup_framework.h` в верхней части каждого из них. CPP файл, в который вы собираетесь добавить функции исправления среды выполнения.
