...
После установки или запуска определенных приложений из Microsoft Store компьютер может неожиданно перезагрузиться из – за следующей ошибки: **0x139 (сбой\_проверки\_безопасностиядра)\_** .
...
### <a name="publisher-and-cert-mismatch-causes-signtool-error-error-signersign-failed--21470248850x8007000b"></a>Несоответствие издателя и сертификата приводит к ошибке SignTool: Сбой Сигнерсигн () (-2147024885/0x8007000b)
...
* Сертификат начинается не после `IMAGE_NT_HEADERS32` структуры для 32-разрядного исполняемого файла или `IMAGE_NT_HEADERS64` после структуры для 64-разрядного исполняемого файла.
