...
ms.locfileid: "67829009"
---
# <a name="prepare-to-package-a-desktop-application"></a>Подготовка к упаковке классического приложения

В этой статье перечислены задачи, которые необходимо знать перед упаковкой приложения для настольных компьютеров. Может не потребоваться значительно Подготовка приложения для процесса упаковки, но если какие-либо элементы ниже действует для приложения, необходимо для решения этой проблемы перед упаковкой. Не забывайте, что лицензирование и автоматическое обновление вашего приложения обеспечивается Microsoft Store, поэтому любые возможности, относящиеся к этим задачам, можно исключить из вашей базы кода.

+ __Приложения .NET требуется версия .NET Framework, предшествующих 4.6.2__. При упаковке приложения .NET, мы рекомендуем требуемой платформы .NET Framework 4.6.2 или более поздней версии. Возможность установки и упакованные настольными приложениями впервые появилась в Windows 10 версии 1607 (также называется Юбилейным обновлением) и включает .NET Framework 4.6.2 по умолчанию, эта версия ОС. Более поздние версии операционной системы включено более поздних версиях платформы .NET Framework. Полный список какие версии .NET, включенных в более поздних версиях Windows 10, см. в разделе [в этой статье](https://docs.microsoft.com/dotnet/framework/migration-guide/versions-and-dependencies).

  Предназначенные для версий платформы .NET Framework, предшествующих 4.6.2 в пакете приложения должен работать в большинстве случаев. Тем не менее если вы устанавливаете, предшествующих 4.6.2, следует полностью протестировать упакованные приложения рабочего стола перед распространением их пользователям.

  + 4.0 - 4.6.1: Приложения, предназначенные для версий платформы .NET Framework должны работать без проблем на 4.6.2 или более поздней версии. Таким образом эти приложения следует установить и выполняться без изменений в Windows 10 версии 1607 или более поздней версии .NET Framework, которая входит в состав операционной системы.

  + 2.0 и 3.5: В ходе тестирования, упакованных настольных приложений, предназначенных для этих версий платформы .NET Framework, обычно работают, но возникают проблемы с производительностью в некоторых сценариях. В порядке эти упакованные приложения для установки и запуска, [компонент .NET Framework 3.5](https://docs.microsoft.com/dotnet/framework/install/dotnet-35-windows-10) должен быть установлен на целевом компьютере (эта функция также включает в себя .NET Framework 2.0 и 3.0). Необходимо также проверить эти приложения, тщательно после их упаковки.
...
+ __Приложению требуется драйвер режима ядра или службы Windows__. Мост для классических приложений подходит для приложения, но он не поддерживает драйвер режима ядра или службы Windows, которое должно выполняться под учетной записью системы. Вместо службы Windows используйте [фоновую задачу](/windows/uwp/launch-resume/create-and-register-a-background-task).
