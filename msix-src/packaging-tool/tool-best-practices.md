...
### <a name="configure-your-msix-packaging-tool-defaults"></a>Настройка параметров по умолчанию для инструмента упаковки MSIX
...
- **Добавлена поддержка MSIX Core при создании пакета** . Этот параметр позволяет добавить поддержку [MSIX Core](../msix-core/msixcore.md) в каждый создаваемый пакет. После выбора этого предложения откроется раскрывающийся список, который позволит указать версию Windows для поддержки. 
