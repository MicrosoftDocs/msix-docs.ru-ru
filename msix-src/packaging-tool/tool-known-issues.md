...
Чтобы устранить эту проблему, откройте **средство упаковки MSIX** и измените приложение с помощью **редактора пакетов**. Откройте манифест и присвойте `MinVersion` атрибуту `TargetDeviceFamily` элемента значение «10.0.16299.0».
...
Возможно, файл открыт или не существует. Чтобы устранить эту проблему, добавьте соответствующий файл или закройте файл, который используется в данный момент. Обратите внимание, что если она `File not Found` открыта, возникнет ошибка. Вместо этого вы получите ошибку `Access Denied` или. `File in Use`
