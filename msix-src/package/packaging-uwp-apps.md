...
1. [Перед формированием пакета приложения](#before-packaging-your-app) Выполните следующие действия, чтобы убедиться, что приложение готово к упаковке.
...
## <a name="before-packaging-your-app"></a>Перед упаковкой приложения
...
### <a name="configure-a-package-with-the-manifest-designer"></a>Настройка пакета с помощью конструктора манифеста
...
3. Теперь можно решить, как нужно настроить приложение. Каждая вкладка содержит сведения о приложении и ссылки на необходимые дополнительные сведения, которые можно настроить.
    ...
    На вкладке **Упаковка** можно ввести сведения о публикации. Здесь также можно выбрать сертификат, который следует использовать для подписи приложения. Все приложения MSIX должны быть подписаны с помощью сертификата.
...
Чтобы распространить приложение на Microsoft Store, рекомендуется создать файл отправки пакета приложения (. мсиксуплоад или. appxupload).
...
2. Щелкните проект правой кнопкой мыши и выберите **опубликовать**->**создать пакеты приложений** (до Visual Studio 2019 версии 16,3, меню **Публикация** называется **Store**). Если этот параметр отключен или не отображается вообще, убедитесь, что проект является универсальным проектом Windows.  
...
2. Загрузите и установите инструменты удаленной отладки для Visual Studio. Эти инструменты используются для удаленного запуска набора сертификации приложений Windows. Дополнительные сведения об этих средствах, включая их загрузку, см. в статье [Запуск приложения MSIX на удаленном компьютере](https://docs.microsoft.com/visualstudio/debugger/run-windows-store-apps-on-a-remote-machine?view=vs-2015).
