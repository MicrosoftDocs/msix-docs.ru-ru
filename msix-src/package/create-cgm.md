...
description: Чтобы подготовить приложение к потоковой установке, необходимо создать карту группы содержимого. Эта статья поможет вам создать и преобразовать файл сопоставления группы содержимого, а также даст некоторые советы и рекомендации.
ms.date: 07/02/2019
author: dianmsft
ms.author: diahar
ms.topic: article
...
> [!IMPORTANT]
> Если вы разрабатываете C++ приложение, необходимо настроить свойства `SourceAppxContentGroupMap.xml`файла. Задайте свойству `Content` значение **true**, а свойству `File Type` — значение **XML File**.
